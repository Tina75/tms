  ...
  methods: {
    ...
    save (name) {
      this.$refs[name].validate((valid) => {
        ...
        params.repairMoney = float.round((this.validate.repairMoney * 100).toFixed(2))
        params.payMoney = float.round((this.validate.payMoney * 100).toFixed(2))
        params.waitPayMoney = float.round((this.validate.waitPayMoney * 100).toFixed(2))
        if (valid) {
          this.loading = true
          if (this.flag === 1) { // 新增
            this.add(params)
