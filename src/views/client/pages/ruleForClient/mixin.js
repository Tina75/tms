            ...
            data: Object.assign({}, _this.ruleDetail, {
              details: _this.ruleDetail.details.map(item => {
                return {
                  ...
                  // 选择车型，件时，起步价，起步量都没有
                  startNum: (_this.ruleDetail.ruleType === '5' || _this.ruleDetail.ruleType === '8') ? null : (item.startNum ? float.round(item.startNum * 100) : ''),
                  // 选择起步量的时候，startPrice的值传startNum的值
                  startPrice: (_this.ruleDetail.ruleType === '5' || _this.ruleDetail.ruleType === '8') ? null : (item.startType === '1' ? (item.startPrice ? float.round(item.startPrice * 100) : '') : (item.startNum ? float.round(item.startNum * 100) : '')),
                  chargeRules: item.chargeRules.map(el => {
                    return {
                      base: (_this.ruleDetail.ruleType === '5' || _this.ruleDetail.ruleType === '8') ? null : (float.round(el.base * 100)),
                      price: float.round(el.price * 100),
                      carType: _this.ruleDetail.ruleType === '5' ? el.carType : null,
                      carLength: _this.ruleDetail.ruleType === '5' ? el.carLength : null,
                      cargoName: _this.ruleDetail.ruleType === '8' ? el.cargoName : null
                    }
                  })
                }
              })
